...
function getOrigemLabel(origem) {
  const valor = (origem || '').toString().trim().toLowerCase();
  if (valor === 'troca') return 'Troca';
  if (valor === 'instalacao') return 'Instalação';
  if (valor === 'formatacao') return 'Formatação';
  return origem || '-';
}
...
function criarLinha(registro) {
  ...
  tr.innerHTML = `
    ...
    <td>
      <strong>${getOrigemLabel(registro.origem)}</strong>
