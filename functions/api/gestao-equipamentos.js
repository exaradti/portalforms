...
async function atualizarRegistro(context, body, usuario, perfil) {
  ...
  if (!['troca', 'instalacao', 'formatacao'].includes(origem)) {
  ...
  const tabelasPorOrigem = {
    troca: 'trocas_ativos',
    instalacao: 'instalacoes_ativos',
    formatacao: 'formatacoes_computadores'
  };
  const tabela = tabelasPorOrigem[origem];
