...
function montarCorpoEmail(dados, usuario, perfil) {
  ...
  const linhas = [
    "Nova instalação de equipamento registrada",
    "",
  ...
  if (dados.ip_ativo_novo) {
    linhas.push(`IP novo: ${dados.ip_ativo_novo}`);
  }

  if (dados.modelo_ativo_novo) {
    linhas.push(`Modelo novo: ${dados.modelo_ativo_novo}`);
  }
  ...
  linhas.push("");
  linhas.push("Observações da instalação:");
  linhas.push(dados.observacoes_instalacao);
...
async function salvarNoSupabase(context, dados) {
  const resposta = await fetch(`${context.env.SUPABASE_URL}/rest/v1/instalacoes_ativos`, {
    method: "POST",
    headers: {
...
async function enviarEmail(context, dados, usuario, perfil) {
  const emailResp = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      subject: `Instalação de equipamento registrada - ${dados.tipo_ativo} - ${dados.setor}`,
  ...
  try {
    ...
    if (perfil && perfil.ativo === false) {
      return Response.json(
        { ok: false, message: "Usuário sem permissão para registrar instalações." },
        { status: 403 }
      );
    }
    ...
    const dados = {
      ...
      observacoes_instalacao: limparTexto(data.observacoes_instalacao),
    ...
    return Response.json({
      ok: true,
      message: `Instalação salva com sucesso para ${nomeTecnico}.`
    });
  } catch (error) {
    return Response.json(
      { ok: false, message: error.message || "Erro interno ao processar a instalação." },
      { status: 500 }
    );
  }
}
